...
void display(void)
{
    ...
    for (int i = 0; i < nObjects; i++)
    {
        ...
        vec3 brightness = so.brightness * 2.0;
...
// Part C1: Materials
static void adjustAmbientDiffuseLight(vec2 ad)
{
...
// Part C2: Materials
static void adjustSpecularLightShine(vec2 ss)
{
...
static void materialMenu(int id)
{
    ...
    {
        toolObj = currObject;
        setToolCallbacks(adjustAmbientDiffuseLight, mat2(1, 0, 0, 1),
                         adjustSpecularLightShine, mat2(1, 0, 0, 50));
    }
    else
    {
...
// Escape key is not released on exit which immediately closes the window when the app starts
